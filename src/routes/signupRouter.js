...
RegisterRouter.get("/user-profile/:id", async (req, res) => {
  try {
    const id = req.params.id
    const User = await register.findOne({ login_id: id });
    if (User) {
      return res.status(400).json({ success: true, error: false, data: User });
    }
    return res.status(400).json({ success: false, error: true, message: "Phone number already exists" });
...
RegisterRouter.post("/update-profile", async (req, res) => {
  try {
    const login_id = req.body.login_id
    var reg = { name: req.body.name, address: req.body.address, phone: req.body.phone, email: req.body.email }
    register.updateOne({ login_id: login_id }, { $set: reg }).then((data) => {
      res.status(201).json({ success: true, error: false, message: "User Profile Updated" });
    })
  } catch (error) {
    res.status(500).json({ success: false, error: true, message: "Something went wrong" });
    console.log(error);
